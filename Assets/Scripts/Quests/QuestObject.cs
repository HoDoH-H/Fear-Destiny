...
public class QuestObject : MonoBehaviour
{
    ...
    [SerializeField] ObjectActions onFail;
    ...
    public void UpdateObjectStatus()
    {
        ...
        else if (onFail != ObjectActions.DoNothing && questList.IsFailed(questToCheck.Name))
        {
            foreach (Transform child in transform)
            {
                if (onFail == ObjectActions.Enable)
                    child.gameObject.SetActive(true);
                else if (onFail == ObjectActions.Disable)
                    child.gameObject.SetActive(false);
            }
        }
    }
